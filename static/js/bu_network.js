class NetworkAnimation {
    constructor() {
        this.svg = document.getElementById('bu_network-svg');
        
        // Performance check - disable on low-memory devices
        if (navigator.deviceMemory && navigator.deviceMemory < 4) {
            console.log('Low memory detected, disabling network animation');
            return;
        }
        
        // Allow users to disable animation
        if (localStorage.getItem('disableAnimation') === 'true') {
            console.log('Network animation disabled by user');
            return;
        }
        ...
        this.frameCount = 0;  // For frame caching
        this.init();
    }
    ...
    createNodes() {
        const nodeCount = 40; // Reduced from 150 (73% fewer nodes for better performance)
        this.nodes = [];
    ...
    createConnections() {
        const maxDistance = 120; // Reduced from 180 (fewer connections for better performance)
        this.connections = [];
    ...
    render() {
        ...
        // Cache connections - only recalculate every 10 frames (huge performance boost)
        ...
        if (this.mouse.x !== null && this.mouse.y !== null) {
            ...
            mouseCircle.setAttribute('fill', '#c92a2a');
        ...
        this.nodes.forEach(node => {
            ...
            circle.setAttribute('fill', '#c92a2a');
